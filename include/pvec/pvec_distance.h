...
inline void dot_batch_avx2(const float* query, const float* matrix,
    ...
    for (; row + 3 < n; row += 4) {
        ...
        // Dual-accumulator per row (8 total): hides 4-cycle FMA latency,
        ...
        float s0 = hsum256_avx2(acc0a);
        float s1 = hsum256_avx2(acc1a);
        float s2 = hsum256_avx2(acc2a);
        float s3 = hsum256_avx2(acc3a);
...
inline void l2_sq_batch_avx2(const float* query, const float* matrix,
    ...
    for (; row + 3 < n; row += 4) {
        ...
        for (; i + 7 < dim; i += 8) {
            ...
            acc0a = _mm256_fmadd_ps(d0, d0, acc0a);
            acc1a = _mm256_fmadd_ps(d1, d1, acc1a);
            acc2a = _mm256_fmadd_ps(d2, d2, acc2a);
            acc3a = _mm256_fmadd_ps(d3, d3, acc3a);
        }
