...
struct palloc_page_pool_impl {
  ...
  // Swap file handle.
#if defined(_WIN32) || defined(_WIN64)
  HANDLE swap_handle = INVALID_HANDLE_VALUE;
#else
  int swap_fd = -1;
#endif

  bool has_swap() const noexcept {
#if defined(_WIN32) || defined(_WIN64)
    return swap_handle != INVALID_HANDLE_VALUE;
#else
    return swap_fd >= 0;
#endif
  }
...
#if defined(_WIN32) || defined(_WIN64)
    if (swap_handle != INVALID_HANDLE_VALUE) {
      ::CloseHandle(swap_handle);
      swap_handle = INVALID_HANDLE_VALUE;
    }
#else
    if (swap_fd >= 0) {
      ::close(swap_fd);
...
#if defined(_WIN32) || defined(_WIN64)
    if (swap_handle != INVALID_HANDLE_VALUE) return true;
  ...
  bool write_page_to_swap(size_t frame, uint64_t page_id) {
    ...
    DWORD written = 0;
    BOOL ok = ::WriteFile(swap_handle, frame_ptr(frame), static_cast<DWORD>(page_size), &written, &ov);
    if (!ok || written != page_size) {
      return false;
    }
#else
  ...
  bool read_page_from_swap(size_t frame, uint64_t page_id) {
    ...
    ov.OffsetHigh = static_cast<DWORD>((off >> 32) & 0xFFFFFFFF);
    DWORD read_bytes = 0;
    BOOL ok = ::ReadFile(swap_handle, frame_ptr(frame), static_cast<DWORD>(page_size), &read_bytes, &ov);
    if (!ok || read_bytes != page_size) {
      return false;
    }
    return true;
#else
