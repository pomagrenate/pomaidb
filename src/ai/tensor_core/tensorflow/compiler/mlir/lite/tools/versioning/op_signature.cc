  ...
  switch (builtin_code) {
    ...
    case BuiltinOperator_FULLY_CONNECTED: {
      ...
      if (weight_quant && weight_quant->scale() &&
          !weight_quant->scale()->size() == 0 && weight_tensor->shape() &&
          !weight_tensor->shape()->size() == 0) {
    ...
    case BuiltinOperator_MUL: {
      if (op->inputs()->size() < 2 || op->outputs()->size() == 0) {
        break;
      }
      ...
      if (input1_quant && input1_quant->scale() &&
          !input1_quant->scale()->size() == 0 && input2_qunt &&
          input2_qunt->scale() && !input2_qunt->scale()->size() == 0 &&
          output_quant && output_quant->scale() &&
          !output_quant->scale()->size() == 0) {
    ...
    case BuiltinOperator_CONV_2D: {
      ...
      if (input_tensor->shape() && !input_tensor->shape()->size() == 0) {
    ...
    case BuiltinOperator_EMBEDDING_LOOKUP: {
      ...
      if (table_quant && table_quant->scale() &&
          !table_quant->scale()->size() == 0 && table_tensor->shape() &&
          !table_tensor->shape()->size() == 0) {
