...
namespace gemmlowp {
  template<typename T, int B> struct FixedPoint {
    typedef T RawType;
    T v;
    FixedPoint() : v(0) {}
    explicit FixedPoint(T _v) : v(_v) {}
    static FixedPoint FromRaw(T _v) { return FixedPoint(_v); }
    T raw() const { return v; }
    FixedPoint operator*(const FixedPoint& other) const { return FixedPoint(v * other.v); }
    FixedPoint operator+(const FixedPoint& other) const { return FixedPoint(v + other.v); }
  };
  template<typename T> struct FixedPointRawTypeTraits { typedef T ScalarRawType; };
  template<typename T> T Dup(T x) { return x; }
  template<typename T> T SaturatingRoundingArithmeticShiftRight(T x, int exponent) { return x >> exponent; }
  template<typename T> T RoundingHalfSum(T a, T b) { return (a + b) / 2; }
  template<int B, typename T, int B2> FixedPoint<T, B> Rescale(FixedPoint<T, B2> x) { return FixedPoint<T, B>(x.v); }
  template<typename T> T RoundingDivideByPOT(T x, int exponent) { 
    if (exponent == 0) return x;
    return (x + (1 << (exponent - 1))) >> exponent; 
  }
  template<typename T> T SaturatingAdd(T a, T b) { return a + b; }
}

#define GEMMLOWP_CHECKED_FIXEDPOINT_CONSTANT(type, value, scalar_value) type::FromRaw(value)
