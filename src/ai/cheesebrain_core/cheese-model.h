...
enum llm_type {
    ...
    LLM_TYPE_57B_A14B,
    LLM_TYPE_17B_16E, // cheese4 Scout
    LLM_TYPE_17B_128E, // cheese4 Maverick
    LLM_TYPE_A13B,
    LLM_TYPE_7B_A1B,
...
struct cheese_layer {
    ...
    struct cheese_layer_posnet posnet;

    struct cheese_layer_convnext convnext;

    struct cheese_layer_shortconv shortconv;

    struct cheese_layer_nextn nextn;
};
...
struct cheese_model {
    ...
    std::vector<cheese_layer> layers;
    ...
    std::unordered_set<cheese_adapter_lora *> loras;

    int64_t t_load_us  = 0;
    int64_t t_start_us = 0;

    explicit cheese_model(const struct cheese_model_params & params);
    ~cheese_model();

    void load_stats  (cheese_model_loader & ml);
    void load_arch   (cheese_model_loader & ml);
    void load_hparams(cheese_model_loader & ml);
    void load_vocab  (cheese_model_loader & ml);
    bool load_tensors(cheese_model_loader & ml); // returns false if cancelled by progress_callback
    ...
    cheese_split_mode split_mode() const;
    ...
    float get_rope_freq_base (const cheese_cparams & cparams, int il) const;
    float get_rope_freq_scale(const cheese_cparams & cparams, int il) const;

    ggml_tensor * get_rope_factors(const cheese_cparams & cparams, int il) const;

    // TODO: move this to new llm_arch_model_i interface
    cheese_memory_i * create_memory(const cheese_memory_params & params, const cheese_cparams & cparams) const;

    // TODO: move this to new llm_arch_model_i interface
    ggml_cgraph * build_graph(const llm_graph_params & params) const;

private:
    cheese_model_params params;
