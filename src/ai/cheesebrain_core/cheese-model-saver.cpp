...
cheese_model_saver::cheese_model_saver(const struct cheese_model & model) : model(model), llm_kv(model.arch) {
    gguf_ctx = gguf_init_empty();
}

cheese_model_saver::~cheese_model_saver() {
    gguf_free(gguf_ctx);
}

void cheese_model_saver::add_kv(const enum llm_kv key, const uint32_t value) {
    gguf_set_val_u32(gguf_ctx, llm_kv(key).c_str(), value);
}

void cheese_model_saver::add_kv(const enum llm_kv key, const int32_t value) {
    gguf_set_val_i32(gguf_ctx, llm_kv(key).c_str(), value);
}

void cheese_model_saver::add_kv(const enum llm_kv key, const float value) {
    gguf_set_val_f32(gguf_ctx, llm_kv(key).c_str(), value);
}

void cheese_model_saver::add_kv(const enum llm_kv key, const bool value) {
    gguf_set_val_bool(gguf_ctx, llm_kv(key).c_str(), value);
}

void cheese_model_saver::add_kv(const enum llm_kv key, const char * value) {
...
void cheese_model_saver::add_kv(const enum llm_kv key, const Container & value, const bool per_layer) {
...
void cheese_model_saver::add_kv(const enum llm_kv key, const std::vector<std::string> & value) {
...
void cheese_model_saver::add_tensor(const struct ggml_tensor * tensor) {
    if (!tensor) {
        return;
    }
...
void cheese_model_saver::add_kv_from_model() {
    const cheese_hparams & hparams = model.hparams;
    const cheese_vocab   & vocab   = model.vocab;
    ...
    for (int32_t id = 0; id < n_vocab; ++id) {
        const cheese_vocab::token_data & token_data = vocab.get_token_data(id);

        tokens[id] = token_data.text;
        scores[id] = token_data.score;

        switch(token_data.attr) {
            case CHEESE_TOKEN_ATTR_UNKNOWN:      token_types[id] = CHEESE_TOKEN_TYPE_UNKNOWN;      break;
            case CHEESE_TOKEN_ATTR_UNUSED:       token_types[id] = CHEESE_TOKEN_TYPE_UNUSED;       break;
            case CHEESE_TOKEN_ATTR_NORMAL:       token_types[id] = CHEESE_TOKEN_TYPE_NORMAL;       break;
            case CHEESE_TOKEN_ATTR_CONTROL:      token_types[id] = CHEESE_TOKEN_TYPE_CONTROL;      break;
            case CHEESE_TOKEN_ATTR_USER_DEFINED: token_types[id] = CHEESE_TOKEN_TYPE_USER_DEFINED; break;
            case CHEESE_TOKEN_ATTR_BYTE:         token_types[id] = CHEESE_TOKEN_TYPE_BYTE;         break;
            case CHEESE_TOKEN_ATTR_UNDEFINED:
            default:                            token_types[id] = CHEESE_TOKEN_TYPE_UNDEFINED;    break;
        }
    }
    ...
    add_kv(LLM_KV_ROPE_SCALING_TYPE,                 cheese_rope_scaling_type_name(hparams.rope_scaling_type_train));
    ...
    // FIXME cheese_token is type i32 but when reading in a GGUF file u32 is expected, not an issue for writing though
...
void cheese_model_saver::add_tensors_from_model() {
    ...
    for (const struct cheese_layer & layer : model.layers) {
...
void cheese_model_saver::save(const std::string & path_model) {
