...
static const char * cheese_expert_gating_func_name(cheese_expert_gating_func_type type) {
    switch (type) {
        case CHEESE_EXPERT_GATING_FUNC_TYPE_SOFTMAX: return "softmax";
        case CHEESE_EXPERT_GATING_FUNC_TYPE_SIGMOID: return "sigmoid";
        default:                                    return "unknown";
    }
}

static const std::map<cheese_rope_scaling_type, const char *> CHEESE_ROPE_SCALING_TYPES = {
    { CHEESE_ROPE_SCALING_TYPE_NONE,       "none"       },
    { CHEESE_ROPE_SCALING_TYPE_LINEAR,     "linear"     },
    { CHEESE_ROPE_SCALING_TYPE_YARN,       "yarn"       },
    { CHEESE_ROPE_SCALING_TYPE_LONGROPE,   "longrope"   },
};

std::string cheese_rope_scaling_type_name(cheese_rope_scaling_type rope_scaling_type) {
    return CHEESE_ROPE_SCALING_TYPES.at(rope_scaling_type);
}

static cheese_rope_scaling_type cheese_rope_scaling_type_from_string(const std::string & name) {
    for (const auto & kv : CHEESE_ROPE_SCALING_TYPES) {
        if (kv.second == name) {
            return (cheese_rope_scaling_type) kv.first;
        }
    }

    return CHEESE_ROPE_SCALING_TYPE_UNSPECIFIED;
}

// checks if the weight tensor can be used with the specified buffer type and device
static bool weight_buft_supported(const cheese_hparams & hparams, ggml_tensor * w, ggml_op op, ggml_backend_buffer_type_t buft, ggml_backend_dev_t dev) {
...
static ggml_backend_buffer_type_t select_weight_buft(const cheese_hparams & hparams, ggml_tensor * tensor, ggml_op op, const buft_list_t & buft_list) {
...
// GPU: split if CHEESE_SPLIT_MODE_ROW -> GPU
static buft_list_t make_gpu_buft_list(ggml_backend_dev_t dev, cheese_split_mode split_mode, const float * tensor_split) {
    buft_list_t buft_list;

    // add the device split buffer type if requested and available
    if (split_mode == CHEESE_SPLIT_MODE_ROW) {
...
struct cheese_model::impl {
    ...
    cheese_mmaps mappings;

    // objects representing data potentially being locked in memory
    cheese_mlocks mlock_bufs;
    cheese_mlocks mlock_mmaps;
...
cheese_model::cheese_model(const cheese_model_params & params) : params(params), pimpl(std::make_unique<impl>()) {
    pimpl->has_tensor_overrides = params.tensor_buft_overrides && params.tensor_buft_overrides[0].pattern;
}

cheese_model::~cheese_model() {
    for (auto * lora : loras) {
        delete lora;
    }
}

void cheese_model::load_stats(cheese_model_loader & ml) {
    pimpl->n_elements = ml.n_elements;
    pimpl->n_bytes = ml.n_bytes;
}

void cheese_model::load_arch(cheese_model_loader & ml) {
    arch = ml.get_arch();
...
void cheese_model::load_hparams(cheese_model_loader & ml) {
    ...
    GGML_ASSERT(hparams.n_expert <= CHEESE_MAX_EXPERTS);
    ...
    hparams.rope_scaling_type_train = cheese_rope_scaling_type_from_string(rope_scaling);
    GGML_ASSERT(hparams.rope_scaling_type_train != CHEESE_ROPE_SCALING_TYPE_UNSPECIFIED);
    ...
    if (hparams.n_head() > 0) {
        ...
        if (arch == LLM_ARCH_CHEESE || arch == LLM_ARCH_DECI || arch == LLM_ARCH_FALCON || arch == LLM_ARCH_CHEESE_EMBED) {
    ...
    switch (arch) {
        ...
        case LLM_ARCH_CHEESE_EMBED:
            {
                ...
                if (hparams.n_expert == 8) {
                    ...
                    switch (hparams.n_layer) {
                        case 16: type = LLM_TYPE_1B; break; // Cheese 3.2 1B
                        case 22: type = LLM_TYPE_1B; break;
                        case 26: type = LLM_TYPE_3B; break;
                        case 28: type = LLM_TYPE_3B; break; // Cheese 3.2 3B
        ...
        case LLM_ARCH_CHEESE4:
            {
                ...
                if (found_swa && hparams.n_swa == 0) {
                    hparams.swa_type             = CHEESE_SWA_TYPE_NONE;
                    hparams.n_no_rope_layer_step = hparams.n_layer; // always use rope
                } else {
                    hparams.swa_type                = CHEESE_SWA_TYPE_CHUNKED;
                    hparams.n_swa                   = 8192;
        ...
        case LLM_ARCH_ARCEE:
            {
                ...
                // Arcee uses the same structure as Cheese
                switch (hparams.n_layer) {
        ...
        case LLM_ARCH_AFMOE:
            {
                ...
                if (hparams.n_swa > 0) {
                    hparams.swa_type = CHEESE_SWA_TYPE_STANDARD;
                ...
                if (hparams.expert_gating_func == CHEESE_EXPERT_GATING_FUNC_TYPE_NONE) {
                    hparams.expert_gating_func = CHEESE_EXPERT_GATING_FUNC_TYPE_SIGMOID;
                }
        ...
        case LLM_ARCH_MODERN_BERT:
            {
                ...
                if (found_swa && hparams.n_swa > 0) {
                    ...
                    hparams.swa_type = CHEESE_SWA_TYPE_SYMMETRIC;
        ...
        case LLM_ARCH_LLADA:
            {
                ml.get_key(LLM_KV_ATTENTION_LAYERNORM_RMS_EPS, hparams.f_norm_rms_eps);
                // LLaDA-8B has 32 layers, similar to Cheese but for diffusion
                switch (hparams.n_layer) {
                    case 32:
                        type = LLM_TYPE_8B;
                        break;
        ...
        case LLM_ARCH_PHI3:
            {
                ...
                if (found_swa && hparams.n_swa > 0) {
                    CHEESE_LOG_WARN("%s: Phi SWA is currently disabled - results might be suboptimal for some models (see %s)\n",
                            __func__, "https://github.com/ggml-org/cheese.cpp/pull/13676");
        ...
        case LLM_ARCH_DEEPSEEK2:
            {
                ...
                if (hparams.expert_gating_func == CHEESE_EXPERT_GATING_FUNC_TYPE_NONE) {
                    ...
                    if ((hparams.n_layer == 47 || hparams.n_layer == 48) && n_vocab == 154880) {
                        ...
                        hparams.expert_gating_func = CHEESE_EXPERT_GATING_FUNC_TYPE_SOFTMAX;
                    }
                }
    ...
    hparams.rope_type = cheese_model_rope_type(this);
}

void cheese_model::load_vocab(cheese_model_loader & ml) {
    const auto kv = LLM_KV(arch);

    vocab.load(ml, kv);
}

bool cheese_model::load_tensors(cheese_model_loader & ml) {
    ...
    CHEESE_LOG_INFO("%s: loading model tensors, this can take a while... (mmap = %s, direct_io = %s)\n",
    ...
    if (all_zero) {
        ...
        for (size_t i = 0; i < n_devices(); ++i) {
            ...
            // fixes: https://github.com/ggml-org/cheese.cpp/issues/18577
            if (free == 0 && total == 0) {
    ...
    auto get_layer_buft_list = [&](int il) -> cheese_model::impl::layer_dev {
        const bool is_swa = il < int(hparams.n_layer) && hparams.is_swa(il);
        if (il < i_gpu_start || (il - i_gpu_start) >= act_gpu_layers) {
            CHEESE_LOG_DEBUG("load_tensors: layer %3d assigned to device %s, is_swa = %d\n", il, ggml_backend_dev_name(cpu_dev), is_swa);
    ...
    const auto TENSOR_DUPLICATED   = cheese_model_loader::TENSOR_DUPLICATED;
    const auto TENSOR_NOT_REQUIRED = cheese_model_loader::TENSOR_NOT_REQUIRED;
    const auto TENSOR_SKIP         = cheese_model_loader::TENSOR_SKIP;
    ...
    // create tensors for the weights
    {
        ...
        auto create_tensor = [&](const LLM_TN_IMPL & tn, const std::initializer_list<int64_t> & ne, int flags) -> ggml_tensor * {
            ...
            if (info.op == GGML_OP_NONE || flags & TENSOR_SKIP) {
                ...
                CHEESE_LOG_WARN("model has unused tensor %s (size = %zu bytes) -- ignoring\n", tn.str().c_str(), nbytes);
            ...
            if (ml.tensor_buft_overrides) {
                ...
                for (const auto * overrides = ml.tensor_buft_overrides; overrides->pattern != nullptr; ++overrides) {
                    ...
                    if (std::regex_search(tensor_name, pattern)) {
                        ...
                        CHEESE_LOG_DEBUG("tensor %s (%zu MiB %s) buffer type overridden to %s\n",
                                tensor_name.c_str(),
        ...
        auto create_tensor_gate_up_exps = [&](cheese_layer & layer, int bid, int64_t n_embd_, int64_t n_ff_, int64_t n_expert_, int flags) {
        ...
        switch (arch) {
            ...
            case LLM_ARCH_CHEESE_EMBED:
                {
                    ...
                    for (int i = 0; i < n_layer; ++i) {
                        ...
                        if (hparams.rope_scaling_type_train == CHEESE_ROPE_SCALING_TYPE_LONGROPE) {
            ...
            case LLM_ARCH_LLADA:
                {
                    ...
                    for (int i = 0; i < n_layer; ++i) {
                        ...
                        // Use separate Q, K, V projections without bias, matching LLaDACheeseBlock
                        layer.wq =
            ...
            case LLM_ARCH_COHERE2:
                ...
                break;
            case LLM_ARCH_OLMO:  // adapted from LLM_ARCH_CHEESE with norm params removed
                {
                    tok_embd = create_tensor(tn(LLM_TENSOR_TOKEN_EMBD, "weight"), {n_embd, n_vocab}, 0);
            ...
            case LLM_ARCH_STEP35:
                {
                    ...
                    for (int i = 0; i < n_layer; ++i) {
                        ...
                        // optional rope factors (cheese3) / longrope tensors
                        if (hparams.rope_scaling_type_train == CHEESE_ROPE_SCALING_TYPE_LONGROPE) {
        ...
        if (n_moved_tensors > 0) {
            CHEESE_LOG_DEBUG("%s: tensor '%s' (%s) (and %d others) cannot be used with preferred buffer type %s, using %s instead\n",
    ...
    std::vector<std::pair<ggml_context *, cheese_buf_map>> ctx_buf_maps;
    ...
    for (auto & [buft, ctx_ptr] : ctx_map) {
        ...
        if (ml.use_mmap && use_mmap_buffer && buffer_from_host_ptr_supported && is_default_buft) {
            ...
            if (use_mlock && ggml_backend_buffer_is_host(buf)) {
                pimpl->mlock_bufs.emplace_back(new cheese_mlock);
    ...
    if (cheese_supports_gpu_offload()) {
        const int n_gpu = std::min(n_gpu_layers, int(hparams.n_layer));

        int n_repeating = n_gpu;
        if (n_repeating > 0) {
            CHEESE_LOG_INFO("%s: offloading output layer to GPU\n", __func__);
            n_repeating--;
        }
        CHEESE_LOG_INFO("%s: offloading %d repeating layers to GPU\n", __func__, n_repeating);

        const int max_backend_supported_layers = hparams.n_layer + 1;
        const int max_offloadable_layers       = hparams.n_layer + 1;

        CHEESE_LOG_INFO("%s: offloaded %d/%d layers to GPU\n", __func__, std::min(n_gpu_layers, max_offloadable_layers), max_backend_supported_layers);
    }

    // print memory requirements per buffer type
    for (auto & [_, bufs] : pimpl->ctxs_bufs) {
        for (auto & buf: bufs) {
            CHEESE_LOG_INFO("%s: %12s model buffer size = %8.2f MiB\n",
...
std::string cheese_model::arch_name() const {
    return llm_arch_name(arch);
}

std::string cheese_model::type_name() const {
    return llm_type_name(type);
}

std::string cheese_model::desc() const {
    return pimpl->desc_str;
}

size_t cheese_model::size() const {
    return pimpl->n_bytes;
}

size_t cheese_model::n_tensors() const {
    return tensors_by_name.size();
}

size_t cheese_model::n_devices() const {
    return devices.size();
}

uint32_t cheese_model::n_gpu_layers() const {
    return params.n_gpu_layers >= 0 ? params.n_gpu_layers : hparams.n_layer + 1;
}

cheese_split_mode cheese_model::split_mode() const {
    return params.split_mode;
}

std::map<ggml_backend_buffer_type_t, size_t> cheese_model::memory_breakdown() const {
...
uint64_t cheese_model::n_elements() const {
    return pimpl->n_elements;
}

void cheese_model::print_info() const {
    const std::string rope_scaling_type = cheese_rope_scaling_type_name(hparams.rope_scaling_type_train);
    ...
    // hparams
    CHEESE_LOG_INFO("%s: arch                  = %s\n",     __func__, arch_name().c_str());
    CHEESE_LOG_INFO("%s: vocab_only            = %d\n",     __func__, hparams.vocab_only);
    CHEESE_LOG_INFO("%s: no_alloc              = %d\n",     __func__, hparams.no_alloc);

    if (!hparams.vocab_only) {
        CHEESE_LOG_INFO("%s: n_ctx_train           = %u\n",     __func__, hparams.n_ctx_train);
        CHEESE_LOG_INFO("%s: n_embd                = %u\n",     __func__, hparams.n_embd);
        CHEESE_LOG_INFO("%s: n_embd_inp            = %u\n",     __func__, hparams.n_embd_inp());
        CHEESE_LOG_INFO("%s: n_layer               = %u\n",     __func__, hparams.n_layer);
        CHEESE_LOG_INFO("%s: n_head                = %s\n",     __func__, print_f([&](uint32_t il) { return hparams.n_head(il);    }, hparams.n_layer).c_str());
        CHEESE_LOG_INFO("%s: n_head_kv             = %s\n",     __func__, print_f([&](uint32_t il) { return hparams.n_head_kv(il); }, hparams.n_layer).c_str());
        CHEESE_LOG_INFO("%s: n_rot                 = %u\n",     __func__, hparams.n_rot);
        CHEESE_LOG_INFO("%s: n_swa                 = %u\n",     __func__, hparams.n_swa);
        CHEESE_LOG_INFO("%s: is_swa_any            = %u\n",     __func__, hparams.is_swa_any());
        CHEESE_LOG_INFO("%s: n_embd_head_k         = %u\n",     __func__, hparams.n_embd_head_k);
        CHEESE_LOG_INFO("%s: n_embd_head_v         = %u\n",     __func__, hparams.n_embd_head_v);
        CHEESE_LOG_INFO("%s: n_gqa                 = %s\n",     __func__, print_f([&](uint32_t il) { return hparams.n_gqa(il);        }, hparams.n_layer).c_str());
        CHEESE_LOG_INFO("%s: n_embd_k_gqa          = %s\n",     __func__, print_f([&](uint32_t il) { return hparams.n_embd_k_gqa(il); }, hparams.n_layer).c_str());
        CHEESE_LOG_INFO("%s: n_embd_v_gqa          = %s\n",     __func__, print_f([&](uint32_t il) { return hparams.n_embd_v_gqa(il); }, hparams.n_layer).c_str());
        CHEESE_LOG_INFO("%s: f_norm_eps            = %.1e\n",   __func__, hparams.f_norm_eps);
        CHEESE_LOG_INFO("%s: f_norm_rms_eps        = %.1e\n",   __func__, hparams.f_norm_rms_eps);
        CHEESE_LOG_INFO("%s: f_clamp_kqv           = %.1e\n",   __func__, hparams.f_clamp_kqv);
        CHEESE_LOG_INFO("%s: f_max_alibi_bias      = %.1e\n",   __func__, hparams.f_max_alibi_bias);
        CHEESE_LOG_INFO("%s: f_logit_scale         = %.1e\n",   __func__, hparams.f_logit_scale);
        CHEESE_LOG_INFO("%s: f_attn_scale          = %.1e\n",   __func__, hparams.f_attention_scale);
        CHEESE_LOG_INFO("%s: n_ff                  = %s\n",     __func__, print_f([&](uint32_t il) { return hparams.n_ff(il); }, hparams.n_layer).c_str());
        CHEESE_LOG_INFO("%s: n_expert              = %u\n",     __func__, hparams.n_expert);
        CHEESE_LOG_INFO("%s: n_expert_used         = %u\n",     __func__, hparams.n_expert_used);
        CHEESE_LOG_INFO("%s: n_expert_groups       = %d\n",     __func__, hparams.n_expert_groups);
        CHEESE_LOG_INFO("%s: n_group_used          = %d\n",     __func__, hparams.n_group_used);
        CHEESE_LOG_INFO("%s: causal attn           = %d\n",     __func__, hparams.causal_attn);
        CHEESE_LOG_INFO("%s: pooling type          = %d\n",     __func__, hparams.pooling_type);
        CHEESE_LOG_INFO("%s: rope type             = %d\n",     __func__, hparams.rope_type);
        CHEESE_LOG_INFO("%s: rope scaling          = %s\n",     __func__, rope_scaling_type.c_str());
        CHEESE_LOG_INFO("%s: freq_base_train       = %.1f\n",   __func__, hparams.rope_freq_base_train);
        CHEESE_LOG_INFO("%s: freq_scale_train      = %g\n",     __func__, hparams.rope_freq_scale_train);
        if (hparams.swa_type != CHEESE_SWA_TYPE_NONE) {
            CHEESE_LOG_INFO("%s: freq_base_swa         = %.1f\n",   __func__, hparams.rope_freq_base_train_swa);
            CHEESE_LOG_INFO("%s: freq_scale_swa        = %g\n",     __func__, hparams.rope_freq_scale_train_swa);
        }
        CHEESE_LOG_INFO("%s: n_ctx_orig_yarn       = %u\n",     __func__, hparams.n_ctx_orig_yarn);
        CHEESE_LOG_INFO("%s: rope_yarn_log_mul     = %.4f\n",   __func__, hparams.rope_yarn_log_mul);
        CHEESE_LOG_INFO("%s: rope_finetuned        = %s\n",     __func__, hparams.rope_finetuned ? "yes" : "unknown");
        // MRoPE (Multi-axis Rotary Position Embedding) sections
        if (const auto & s = hparams.rope_sections; s[0] || s[1] || s[2] || s[3]) {
            CHEESE_LOG_INFO("%s: mrope sections        = [%d, %d, %d, %d]\n", __func__, s[0], s[1], s[2], s[3]);
        }
        if (!classifier_labels.empty()) {
            CHEESE_LOG_INFO("%s: n_cls_out             = %u\n", __func__, hparams.n_cls_out);

            size_t i = 0;
            for (auto label : classifier_labels) {
                CHEESE_LOG_INFO("%s: cls_label[%2zu]         = %s\n", __func__, i++, label.c_str());
            }
        }
    }
    ...
    if (arch == LLM_ARCH_MAMBA ||
        ...
        CHEESE_LOG_INFO("%s: ssm_d_conv            = %u\n",     __func__, hparams.ssm_d_conv);
        CHEESE_LOG_INFO("%s: ssm_d_inner           = %u\n",     __func__, hparams.ssm_d_inner);
        CHEESE_LOG_INFO("%s: ssm_d_state           = %u\n",     __func__, hparams.ssm_d_state);
        CHEESE_LOG_INFO("%s: ssm_dt_rank           = %u\n",     __func__, hparams.ssm_dt_rank);
        CHEESE_LOG_INFO("%s: ssm_n_group           = %u\n",     __func__, hparams.ssm_n_group);
        CHEESE_LOG_INFO("%s: ssm_dt_b_c_rms        = %d\n",     __func__, hparams.ssm_dt_b_c_rms);
    }

    CHEESE_LOG_INFO("%s: model type            = %s\n",     __func__, type_name().c_str());
    if (pimpl->n_elements >= 1e12) {
        CHEESE_LOG_INFO("%s: model params          = %.2f T\n", __func__, pimpl->n_elements*1e-12);
    } else if (pimpl->n_elements >= 1e9) {
        CHEESE_LOG_INFO("%s: model params          = %.2f B\n", __func__, pimpl->n_elements*1e-9);
    } else if (pimpl->n_elements >= 1e6) {
        CHEESE_LOG_INFO("%s: model params          = %.2f M\n", __func__, pimpl->n_elements*1e-6);
    } else {
        CHEESE_LOG_INFO("%s: model params          = %.2f K\n", __func__, pimpl->n_elements*1e-3);
    }

    // general kv
    CHEESE_LOG_INFO("%s: general.name          = %s\n",    __func__, name.c_str());

    if (arch == LLM_ARCH_DEEPSEEK) {
        CHEESE_LOG_INFO("%s: n_layer_dense_lead    = %d\n",     __func__, hparams.n_layer_dense_lead);
        CHEESE_LOG_INFO("%s: n_ff_exp              = %d\n",     __func__, hparams.n_ff_exp);
        CHEESE_LOG_INFO("%s: n_expert_shared       = %d\n",     __func__, hparams.n_expert_shared);
        CHEESE_LOG_INFO("%s: expert_weights_scale  = %.1f\n",   __func__, hparams.expert_weights_scale);
    }

    if (arch == LLM_ARCH_DEEPSEEK2 || arch == LLM_ARCH_GLM_DSA) {
        CHEESE_LOG_INFO("%s: n_layer_dense_lead    = %d\n",     __func__, hparams.n_layer_dense_lead);
        CHEESE_LOG_INFO("%s: n_lora_q              = %d\n",     __func__, hparams.n_lora_q);
        CHEESE_LOG_INFO("%s: n_lora_kv             = %d\n",     __func__, hparams.n_lora_kv);
        CHEESE_LOG_INFO("%s: n_embd_head_k_mla     = %d\n",     __func__, hparams.n_embd_head_k_mla());
        CHEESE_LOG_INFO("%s: n_embd_head_v_mla     = %d\n",     __func__, hparams.n_embd_head_v_mla());
        CHEESE_LOG_INFO("%s: n_ff_exp              = %d\n",     __func__, hparams.n_ff_exp);
        CHEESE_LOG_INFO("%s: n_expert_shared       = %d\n",     __func__, hparams.n_expert_shared);
        CHEESE_LOG_INFO("%s: expert_weights_scale  = %.1f\n",   __func__, hparams.expert_weights_scale);
        CHEESE_LOG_INFO("%s: expert_weights_norm   = %d\n",     __func__, hparams.expert_weights_norm);
        CHEESE_LOG_INFO("%s: expert_gating_func    = %s\n",     __func__, cheese_expert_gating_func_name((cheese_expert_gating_func_type) hparams.expert_gating_func));
    }

    if (arch == LLM_ARCH_QWEN2MOE) {
        CHEESE_LOG_INFO("%s: n_ff_exp              = %d\n",     __func__, hparams.n_ff_exp);
        CHEESE_LOG_INFO("%s: n_ff_shexp            = %d\n",     __func__, hparams.n_ff_shexp);
    }
    ...
    if (arch == LLM_ARCH_MINICPM ||
        ...
        arch == LLM_ARCH_GRANITE_HYBRID ||
        arch == LLM_ARCH_NEMOTRON_H_MOE) {
        CHEESE_LOG_INFO("%s: f_embedding_scale     = %f\n", __func__, hparams.f_embedding_scale);
        CHEESE_LOG_INFO("%s: f_residual_scale      = %f\n", __func__, hparams.f_residual_scale);
        CHEESE_LOG_INFO("%s: f_attention_scale     = %f\n", __func__, hparams.f_attention_scale);
        CHEESE_LOG_INFO("%s: n_ff_shexp            = %d\n", __func__, hparams.n_ff_shexp);
    }
    ...
    if (arch == LLM_ARCH_BAILINGMOE2) {
        ...
        CHEESE_LOG_INFO("%s: expert_weights_norm   = %d\n",     __func__, hparams.expert_weights_norm);
        CHEESE_LOG_INFO("%s: expert_gating_func    = %s\n",     __func__, cheese_expert_gating_func_name((cheese_expert_gating_func_type) hparams.expert_gating_func));
        CHEESE_LOG_INFO("%s: nextn_predict_layers  = %d\n",     __func__, hparams.nextn_predict_layers);
    }
    ...
    if (arch == LLM_ARCH_GROVEMOE) {
        CHEESE_LOG_INFO("%s: n_ff_exp              = %d\n",     __func__, hparams.n_ff_exp);
        CHEESE_LOG_INFO("%s: n_ff_chexp            = %d\n",     __func__, hparams.n_ff_chexp);
        CHEESE_LOG_INFO("%s: n_group_experts       = %d\n",     __func__, hparams.n_group_experts);
        CHEESE_LOG_INFO("%s: expert_group_scale    = %.2f\n",   __func__, hparams.expert_group_scale);
    }

    vocab.print_info();
}

ggml_backend_dev_t cheese_model::dev_layer(int il) const {
    return pimpl->dev_layer.at(il).dev;
}

ggml_backend_dev_t cheese_model::dev_output() const {
...
ggml_backend_buffer_type_t cheese_model::select_buft(int il) const {
    return ::select_buft(
...
bool cheese_model::has_tensor_overrides() const {
    return pimpl->has_tensor_overrides;
}

const ggml_tensor * cheese_model::get_tensor(const char * name) const {
...
float cheese_model::get_rope_freq_base (const cheese_cparams & cparams, int il) const {
    return hparams.is_swa(il) ? hparams.rope_freq_base_train_swa : cparams.rope_freq_base;
}

float cheese_model::get_rope_freq_scale(const cheese_cparams & cparams, int il) const {
    return hparams.is_swa(il) ? hparams.rope_freq_scale_train_swa : cparams.rope_freq_scale;
}

ggml_tensor * cheese_model::get_rope_factors(const cheese_cparams & cparams, int il) const {
...
cheese_memory_i * cheese_model::create_memory(const cheese_memory_params & params, const cheese_cparams & cparams) const {
    cheese_memory_i * res;
        ...
        default:
            {
                if (llm_arch_is_recurrent(arch)) {
                    res = new cheese_memory_recurrent(
                            *this,
                            GGML_TYPE_F32,
                            GGML_TYPE_F32,
                            cparams.offload_kqv,
                    ...
                    cheese_memory_hybrid::layer_filter_cb filter_attn = nullptr;
                    cheese_memory_hybrid::layer_filter_cb filter_recr = nullptr;
                    ...
                    if (hparams.swa_type != CHEESE_SWA_TYPE_NONE) {
                        ...
                        res = new cheese_memory_hybrid_iswa(
                            /* model             */ *this,
                    ...
                    cheese_memory_i::layer_reuse_cb reuse = nullptr;
...
ggml_cgraph * cheese_model::build_graph(const llm_graph_params & params) const {
    std::unique_ptr<llm_graph_context> llm;

    switch (arch) {
        case LLM_ARCH_CHEESE:
            {
                llm = std::make_unique<llm_build_cheese<false>>(*this, params);
            } break;
        case LLM_ARCH_CHEESE4:
            {
                if (hparams.swa_type == CHEESE_SWA_TYPE_NONE) {
                    ...
                    llm = std::make_unique<llm_build_cheese_iswa>(*this, params);
                }
            } break;
        case LLM_ARCH_CHEESE_EMBED:
            {
                llm = std::make_unique<llm_build_cheese<true>>(*this, params);
            } break;
        case LLM_ARCH_MAINCODER:
            {
        ...
        case LLM_ARCH_GEMMA3:
            {
                if (hparams.swa_type == CHEESE_SWA_TYPE_STANDARD) {
...
cheese_model_params cheese_model_default_params() {
    cheese_model_params result = {
        /*.devices                     =*/ nullptr,
        /*.tensor_buft_overrides       =*/ nullptr,
        /*.n_gpu_layers                =*/ -1,
        /*.split_mode                  =*/ CHEESE_SPLIT_MODE_LAYER,
        /*.main_gpu                    =*/ 0,
