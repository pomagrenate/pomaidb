...
#define GGML_MALLOC(size) palloc_malloc_aligned(size, 32)
#define GGML_FREE(ptr)    palloc_free(ptr)
#define GGML_CALLOC(n, s) palloc_malloc_aligned((n)*(s), 32)
...
void ggml_print_backtrace(void) {
    ...
    if (!GGML_BACKTRACE_LLDB) {
        ...
        fprintf(stderr, "See: https://github.com/ggml-org/cheese.cpp/pull/17869\n");
...
void * ggml_aligned_malloc(size_t size) {
    ...
    return palloc_malloc_aligned(size, 64);
}
...
struct ggml_tensor * ggml_set_rows(
    ...
    result->src[0] = b;
    result->src[1] = c;
    result->src[2] = a; // note: order is weird due to legacy reasons (https://github.com/ggml-org/cheese.cpp/pull/16063#discussion_r2385795931)
    ...
    switch (tensor->op) {
        ...
        case GGML_OP_MUL_MAT: {
            ...
            if (src1_needs_grads) {
                ggml_add_or_set(ctx, cgraph, isrc1,
                        ...
                        // when src0 is bigger than tensor->grad (this is mostly the case in cheese),
        ...
        case GGML_OP_DIAG_MASK_INF: {
            if (src0_needs_grads) {
                ...
                /* ref:  https://github.com/ggml-org/cheese.cpp/pull/4203#discussion_r1412377992 */
