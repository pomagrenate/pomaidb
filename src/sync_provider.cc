...
FileWalSyncReceiver::FileWalSyncReceiver(const std::string& path, Env* env)
    : path_(path), env_(env ? env : Env::Default()) {}

FileWalSyncReceiver::~FileWalSyncReceiver() {
    if (file_) {
        (void)file_->Flush();
        (void)file_->Sync();
        (void)file_->Close();
...
Status FileWalSyncReceiver::Receive(const WalEntry& entry) {
    ...
    const size_t raw_bytes = entry.raw_data.size();
    const size_t tenant_bytes = entry.meta.tenant.size();
    const size_t total_sz = 8 + 1 + 8 + 4 + vec_bytes + 4 + raw_bytes + 4 + tenant_bytes;
    std::vector<uint8_t> buf(total_sz);
    uint8_t* p = buf.data();

    std::memcpy(p, &entry.lsn, 8); p += 8;
    std::memcpy(p, &entry.op, 1); p += 1;
    std::memcpy(p, &entry.id, 8); p += 8;
    std::memcpy(p, &entry.dim, 4); p += 4;
    if (vec_bytes > 0 && entry.vec.data()) {
        std::memcpy(p, entry.vec.data(), vec_bytes);
    }
    p += vec_bytes;
    uint32_t rlen = static_cast<uint32_t>(raw_bytes);
    std::memcpy(p, &rlen, 4); p += 4;
    if (rlen > 0) {
        std::memcpy(p, entry.raw_data.data(), rlen);
        p += rlen;
    }
    uint32_t tlen = static_cast<uint32_t>(tenant_bytes);
    std::memcpy(p, &tlen, 4); p += 4;
    if (tlen > 0) {
        std::memcpy(p, entry.meta.tenant.data(), tlen);
        p += tlen;
    }

    auto st = file_->Append(Slice(reinterpret_cast<const char*>(buf.data()), buf.size()));
    if (!st.ok()) return st;
    count_++;
    last_lsn_ = entry.lsn;
    return Status::Ok();
...
Status FileWalSyncReceiver::Flush() {
