...
void RunEnvA(EnvReport* report) {
  ...
  report->limit_enforced     = "100k vectors (1536-dim)";
  ...
  IngestResult r = IngestAndVerify("/tmp/benchmark_a_env_a", kEnvATargetVectors);
  ...
  if (r.verified == kEnvATargetVectors && r.ingested == kEnvATargetVectors) {
    report->passed = 1;
...
void RunEnvB(EnvReport* report) {
  ...
  report->limit_enforced     = "50k × 50 cycles (fresh DB per cycle)";
  report->vectors_allocated  = 0;
  report->vectors_verified   = 0;
  report->throughput_vec_per_sec = 0.0;
  report->peak_rss_bytes     = 0;
  ...
  for (int cycle = 0; cycle < kEnvBCycles; ++cycle) {
    std::string path = std::string("/tmp/benchmark_a_env_b_") + std::to_string(cycle);
    IngestResult r = IngestAndVerify(path, kEnvBVectorsPerCycle);
    total_ingested += r.ingested;
    last_verified = r.verified;
    if (r.verified != kEnvBVectorsPerCycle || r.ingested != kEnvBVectorsPerCycle)
      all_ok = false;
  ...
  if (!all_ok) {
    report->passed = 0;
    report->message = "FAIL: one or more cycles had verify count != 50k";
    return;
  }
  if (rss_after_first <= 0) {
    report->passed  = 1;
    report->message = "Peak RSS stable (no leak); all 50 cycles verified";
  } else {
...
void RunEnvC(EnvReport* report) {
  ...
  report->limit_enforced     = "1M vectors (1536-dim)";
  report->vectors_allocated  = 0;
  report->vectors_verified   = 0;
  report->throughput_vec_per_sec = 0.0;
  report->peak_rss_bytes     = 0;
  ...
  IngestResult r = IngestAndVerify("/tmp/benchmark_a_env_c", kEnvCTargetVectors);
  report->vectors_allocated  = r.ingested;
  report->vectors_verified   = r.verified;
  report->throughput_vec_per_sec = r.throughput_vec_per_sec;
  report->peak_rss_bytes    = GetPeakRssBytes();

  if (r.verified == kEnvCTargetVectors && r.ingested == kEnvCTargetVectors) {
    report->passed = 1;
    report->message = "Bulk index build completed; inspect count matches";
  } else if (r.verified != r.ingested) {
    report->message = "FAIL: inspect count mismatch";
  } else {
    report->message = "FAIL: incomplete ingest (expected 1M)";
  }
}
